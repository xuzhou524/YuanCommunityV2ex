...
extension UserModel{
    ...
    class func twoFALogin(code:String,
        ...
        YuanCommunUser.sharedInstance.getOnce { (response) in
            if(response.success){
                let prames = [
                    "code":code,
                    "once":YuanCommunUser.sharedInstance.once!
                    ] as [String:Any]
                let url = V2EXURL + "2fa"
    ...
    class func dailyRedeem() {
        YuanCommunUser.sharedInstance.getOnce { (response) -> Void in
            if response.success {
                Alamofire.request(V2EXURL + "mission/daily/redeem?once=" + YuanCommunUser.sharedInstance.once! , headers: MOBILE_CLIENT_HEADERS).responseJiHtml{ (response) in
                    if let jiHtml = response .result.value{
